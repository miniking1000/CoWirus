...
public class cure implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args){
        if (sender instanceof Player) {
            ...
            if (args.length == 0) {
                message.send(player, "Использование: /cure Игрок - полностью снимает с игрока заражение");
                ...
                if (target.hasMetadata("when_infected")) {
                    ...
                    message.send(player, "Заражение с игрока &6" + target.getName() + "&f было снято.");
                } else {
                    message.send(player, "Игрок не заражен!");
                }
            }
        } else{
            ...
            Plugin plugin = Bukkit.getPluginManager().getPlugin("CoWirus");
            if (args.length == 0) {
                logger.info("Использование: /cure Игрок - полностью снимает с игрока заражение");
            } else if (args.length == 1) {
                Player target = server.getPlayer(args[0]);
                if (!Bukkit.getOnlinePlayers().contains(target)) {
                ...
                if (target.hasMetadata("when_infected")) {
                    target.removeMetadata("when_infected", plugin);
                    plugin2.updInfected(target.getName(), null);
                    logger.info("Заражение с игрока " + target.getName() + " было снято.");
                } else {
                    logger.info("Игрок не заражен!");
                }
            }
        }
        return true;
